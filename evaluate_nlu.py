...
def evaluate_intents(targets, predictions):  # pragma: no cover
    ...
    with open("Evaluation/confusion_matrix_nlu.txt", "w+") as ff:
        np.savetxt(ff, cnf_matrix, fmt='%d')

    with open("Evaluation/confusion_matrix_labels.txt", "w+") as ff:
        ff.write(str(labels))
...
def determine_token_labels(token, entities):
    ...
    # we don't really care if they overlap
